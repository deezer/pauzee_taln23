...
from models.data_module import LABEL_ENCODER_UNBABEL, LABEL_ENCODER_PAUZEE
...
class PunctuationPredictor(pl.LightningModule):
    ...
    def __init__(self, hparams: Namespace):
        ...
        self.punct_head = nn.Linear(
            2 * self.encoder.config.hidden_size, len(LABEL_ENCODER_PAUZEE)
        )
        self.binary_f1 = F1Score(num_classes=1, multiclass=False)
        self.micro_f1 = F1Score(num_classes=len(LABEL_ENCODER_PAUZEE), average="micro")
        self.macro_f1 = F1Score(num_classes=len(LABEL_ENCODER_PAUZEE), average="macro")
    ...
    def predict(self, batch):
        ...
        mask = (input_ids != self.tokenizer.pad_token_id).bool().cuda()
        mask[:, 0] = 1

        label_decoder = {v: k for k, v in LABEL_ENCODER_PAUZEE.items()}
        with torch.no_grad():
            batch = [model_input.cuda() for model_input in batch]
            output = self.forward(*batch)
            binary_pred = torch.topk(output.binary_logits, 1)[1].view(-1).cuda()
            punct_pred = torch.topk(output.punct_logits, 1)[1].view(-1).cuda()
